...
Elm.Store.make = function (_elm) {
   ...
   var NoOp = {ctor: "NoOp"};
   var Model = F2(function (a,b) {    return {value: a,count: b};});
   var init = function (value) {    return {ctor: "_Tuple2",_0: {value: value,count: 1},_1: $Effects.none};};
   ...
   var events = $Signal.mergeMany(_U.list([A2($Signal.map,$Basics.always(Decrement),decrement)
                                          ...
                                          ,A2($Signal.map,$Basics.always(Decrement),clock)]));
   var app = $StartApp.start({init: init(0),update: update,view: view,inputs: _U.list([events])});
   var out = Elm.Native.Port.make(_elm).outboundSignal("out",function (v) {    return {value: v.value,count: v.count};},app.model);
